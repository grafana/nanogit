...
var _ = Describe("Authorization", func() {
	...
	It("should successfully authorize with basic auth", func() {
		...
		authClient, err := nanogit.NewHTTPClient(remoteURL, client.WithBasicAuth(user.Username, user.Password))
	...
	It("should fail authorization with wrong credentials", func() {
		...
		unauthorizedClient, err := nanogit.NewHTTPClient(remoteURL, client.WithBasicAuth("wronguser", "wrongpass"))
		Expect(err).NotTo(HaveOccurred())
	...
	It("should successfully authorize with access token", func() {
		...
		tokenClient, err := nanogit.NewHTTPClient(remoteURL, client.WithTokenAuth(token))
		Expect(err).NotTo(HaveOccurred())
	...
	It("should fail authorization with invalid token", func() {
		...
		invalidClient, err := nanogit.NewHTTPClient(remoteURL, client.WithTokenAuth(invalidToken))
		Expect(err).NotTo(HaveOccurred())
