...
var _ = Describe("Blobs", func() {
	Context("GetBlob operations", func() {
		...
		It("should get blob with valid hash", func() {
			...
			Expect(local.CreateFile("blob.txt", string(testContent))).To(Succeed())
	...
	Context("GetBlobByPath with nested directories", func() {
		...
		BeforeEach(func() {
			...
			Expect(local.CreateFile("root.txt", "root file content")).To(Succeed())
			Expect(local.CreateFile("dir1/file1.txt", "dir1 file content")).To(Succeed())
			Expect(local.CreateFile("dir1/subdir1/nested.txt", "deeply nested content")).To(Succeed())
			Expect(local.CreateFile("dir2/file2.txt", "dir2 file content")).To(Succeed())
	...
	Context("Large blob operations", func() {
		...
		It("should handle extra large dashboard file (3.7MB)", func() {
			...
			Expect(local.CreateFile("xlarge-dashboard.json", string(dashboardContent))).To(Succeed())
