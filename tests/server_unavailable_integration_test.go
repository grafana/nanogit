...
var _ = Describe("Server Unavailable Error Handling", func() {
	...
	Context("Error Propagation Through Client Methods", func() {
		...
		PIt("should propagate ServerUnavailableError from ReceivePack", func() {
			By("Setting up a real repository first")
			client, _, local, _ := QuickSetup()

			By("Creating initial commit")
			...
			By("Creating a file to push")
			_, err = writer.CreateBlob(ctx, "newfile.txt", []byte("new content"))
			Expect(err).NotTo(HaveOccurred())

			_, err = writer.Commit(ctx, "Add new file", nanogit.Author{
				Name:  "Test Author",
			...
			By("Creating a mock server that returns 500 for receive-pack")
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				// For info/refs, return success
				w.WriteHeader(http.StatusOK)
				w.Write([]byte("000eversion 2\n0000"))
			}))
			defer server.Close()

			By("Creating a new client pointing to mock server")
			mockClient, err := nanogit.NewHTTPClient(server.URL + "/test.git")
			Expect(err).NotTo(HaveOccurred())

			By("Creating a new writer with mock client")
			mockWriter, err := mockClient.NewStagedWriter(ctx, ref)
			Expect(err).NotTo(HaveOccurred())

			By("Creating a file to push")
			_, err = mockWriter.CreateBlob(ctx, "newfile.txt", []byte("new content"))
			Expect(err).NotTo(HaveOccurred())

			_, err = mockWriter.Commit(ctx, "Add new file", nanogit.Author{
				Name:  "Test Author",
				Email: "test@example.com",
			}, nanogit.Committer{
				Name:  "Test Committer",
			...
			By("Calling Push which uses ReceivePack internally should return ServerUnavailableError")
			err = mockWriter.Push(ctx)
			Expect(err).To(HaveOccurred())
			Expect(errors.Is(err, nanogit.ErrServerUnavailable)).To(BeTrue(), "error should propagate through Push")
