...
var _ = Describe("Protocol Error Scenarios", func() {
	...
	Context("Reference Update Failures", func() {
		It("should handle old hash reference update failure", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			initialHash, err := hash.FromHex(output)
			Expect(err).NotTo(HaveOccurred())

			By("Creating a new commit locally to advance the remote")
			_, err = local.Git("commit", "--allow-empty", "-m", "Second commit")
			Expect(err).NotTo(HaveOccurred())
		...
		It("should handle non-fast-forward reference update failure", func() {
			...
			output, err := local.Git("rev-parse", "HEAD")
			Expect(err).NotTo(HaveOccurred())
			firstCommitHash, err := hash.FromHex(output)
			Expect(err).NotTo(HaveOccurred())

			By("Creating a divergent commit history locally")
			_, err = local.Git("commit", "--allow-empty", "-m", "Local commit")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("push", "origin", "main")
			Expect(err).NotTo(HaveOccurred())

			By("Resetting local to first commit and creating conflicting commit")
			_, err = local.Git("reset", "--hard", firstCommitHash.String())
			Expect(err).NotTo(HaveOccurred())
			Expect(local.CreateFile("file2.txt", "conflicting content")).To(Succeed())
			_, err = local.Git("add", "file2.txt")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("commit", "-m", "Conflicting commit")
			Expect(err).NotTo(HaveOccurred())
	...
	Context("Git Server Error Messages", func() {
		It("should handle server-side validation errors", func() {
			...
			output, err := local.Git("rev-parse", "HEAD")
			Expect(err).NotTo(HaveOccurred())
			currentHash, err := hash.FromHex(output)
			Expect(err).NotTo(HaveOccurred())

			// Create a commit locally to advance the remote
			Expect(local.CreateFile("server-test.txt", "server test content")).To(Succeed())
			_, err = local.Git("add", "server-test.txt")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("commit", "-m", "Server test commit")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("push", "origin", "main")
			Expect(err).NotTo(HaveOccurred())
	...
	Context("Protocol Error Recovery", func() {
		It("should handle transient errors gracefully", func() {
			...
			Expect(local.CreateFile("recovery-test.txt", "recovery test")).To(Succeed())
			_, err := local.Git("add", "recovery-test.txt")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("commit", "-m", "Recovery test commit")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("branch", "-M", "main")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("push", "-u", "origin", "main", "--force")
	...
	Context("Protocol Error Edge Cases", func() {
		It("should handle invalid object scenarios", func() {
			...
			By("Setting up repository")
			Expect(local.CreateFile("edge-case.txt", "edge case test")).To(Succeed())
			_, err := local.Git("add", "edge-case.txt")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("commit", "-m", "Edge case test")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("branch", "-M", "main")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("push", "-u", "origin", "main", "--force")
		...
		It("should handle empty and boundary condition operations", func() {
			...
			By("Setting up repository")
			Expect(local.CreateFile("boundary.txt", "boundary test")).To(Succeed())
			_, err := local.Git("add", "boundary.txt")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("commit", "-m", "Boundary test")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("branch", "-M", "main")
			Expect(err).NotTo(HaveOccurred())
			_, err = local.Git("push", "-u", "origin", "main", "--force")
