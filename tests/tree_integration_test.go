...
var _ = Describe("Trees", func() {
	...
	Context("GetFlatTree with large repository (700+ files, 90+ folders)", func() {
		var (
			client     nanogit.Client
			local      *LocalGitRepo
			commitHash hash.Hash
		...
		BeforeEach(func() {
			...
			By("Creating a large repository structure matching the failing case")
			// This simulates the real-world scenario from GitHub issue #116880
			// - 700+ dashboard JSON files
			// - 90+ folders
			// - 3-4 levels deep (qa/ and prod/ folders with nested subdirectories)

			fileCount := 0
			folderCount := 0

			// Create structure: qa/ and prod/ folders with nested subdirectories
			for _, env := range []string{"qa", "prod"} {
				// Create 45 folders per environment (90 total)
				for i := 1; i <= 45; i++ {
					folderName := fmt.Sprintf("Service-%d", i)
					folderPath := fmt.Sprintf("%s/%s", env, folderName)
					local.CreateDirPath(folderPath)
					folderCount++

					// Create 7-8 dashboard JSON files per folder (~700 total)
					for j := 1; j <= 8; j++ {
						fileName := fmt.Sprintf("%s/dashboard-%d.json", folderPath, j)
						content := fmt.Sprintf(`{
  "dashboard": {
    "title": "Dashboard %d in %s",
    "uid": "%s-svc%d-dash%d",
    "panels": []
  }
}`, j, folderPath, env, i, j)
						local.CreateFile(fileName, content)
						fileCount++
					}

					// Create nested subfolders every 10th folder (3-4 levels deep)
					if i%10 == 0 {
						subfolderA := folderPath + "/Subfolder-A"
						subfolderB := subfolderA + "/Subfolder-B"
						local.CreateDirPath(subfolderB)
						folderCount += 2

						nestedFile := subfolderB + "/nested-dashboard.json"
						nestedContent := fmt.Sprintf(`{
  "dashboard": {
    "title": "Nested Dashboard in %s",
    "uid": "%s-svc%d-nested",
    "panels": []
  }
}`, folderPath, env, i)
						local.CreateFile(nestedFile, nestedContent)
						fileCount++
					}
				}
			}

			By("Committing the large structure")
			local.Git("add", ".")
			local.Git("commit", "-m", "Initial commit with 700+ dashboards across 90+ folders")
			local.Git("branch", "-M", "main")
			local.Git("push", "origin", "main", "--force")
			...
			By("Repository statistics")
			GinkgoWriter.Printf("Created repository with %d files across %d folders\n", fileCount, folderCount)
		})

		It("should retrieve large flat tree structure without missing tree errors", func() {
			By("Fetching the flat tree for the entire large repository")
			flatTree, err := client.GetFlatTree(ctx, commitHash)

			By("Verifying no 'tree object not found' errors occurred")
			Expect(err).NotTo(HaveOccurred(), "Should not encounter missing tree object errors")
			Expect(flatTree).NotTo(BeNil())

			By("Verifying all files are present in flat tree")
			...
			By("Checking file count")
			// We should have 720+ files (360 per environment)
			// - 45 folders × 8 dashboards = 360 dashboards per env
			// - Plus 9 nested dashboards (every 10th folder) = 9 per env
			// Total: (360 + 9) × 2 = 738 files, plus test.txt = 739
			Expect(fileCount).To(BeNumerically(">=", 720), "Should have at least 720 files")

			By("Checking directory count")
			// We should have 90+ directories
			// - 45 folders × 2 environments = 90 base folders
			// - Plus 9 Subfolder-A per env = 18
			// - Plus 9 Subfolder-B per env = 18
			// Total: 90 + 18 + 18 = 126 directories
			Expect(dirCount).To(BeNumerically(">=", 90), "Should have at least 90 directories")

			By("Verifying specific nested paths are accessible")
			// Check for a deeply nested file (4 levels deep)
			foundDeepNested := false
			for _, entry := range flatTree.Entries {
				if strings.Contains(entry.Path, "Subfolder-A/Subfolder-B/nested-dashboard.json") {
					foundDeepNested = true
					Expect(entry.Type).To(Equal(protocol.ObjectTypeBlob))
					break
				}
			}
			Expect(foundDeepNested).To(BeTrue(), "Should find deeply nested files (4 levels deep)")

			By("Test passed - no missing tree objects!")
			GinkgoWriter.Printf("✓ Successfully retrieved flat tree with %d files and %d directories\n", fileCount, dirCount)
		})
	})
})
