...
func (mr *MultiplexedReader) Close() error {
	mr.logger.Debug("Closing multiplexed reader")
	if mr.parser != nil {
		return mr.parser.Close()
	}
	return nil
}
...
outer:
	for {
		...
		case "packfile":
			...
			defer func() {
				if closeErr := multiplexedReader.Close(); closeErr != nil {
					logger.Debug("Error closing multiplexed reader", "error", closeErr)
				}
			}()
			...
			if err != nil {
				// If there's no packfile signature, it means there was no actual pack data
				// (e.g., only progress messages), which is valid
				if errors.Is(err, ErrNoPackfileSignature) {
					logger.Debug("No packfile data found (only progress messages), returning empty response")
					return fr, nil
				}
