...
// getSpecificObject fetches only the specific object(s) requested without using filters.
// This is more targeted than getObjects which might include additional objects based on filters.
func (c *httpClient) getSpecificObject(ctx context.Context, want ...hash.Hash) (map[string]*protocol.PackfileObject, error) {
	packs := []protocol.Pack{
		protocol.PackLine("command=fetch\n"),
		protocol.PackLine("object-format=sha1\n"),
		protocol.SpecialPack(protocol.DelimeterPacket),
		protocol.PackLine("no-progress\n"),
		// Use shallow to prevent Git from sending entire commit history
		// This tells Git we don't want history before this commit
	}
